...
export function ThemeSelectorModal({ open, onOpenChange }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="glass-card sm:max-w-xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <p className="text-xs text-muted-foreground">
            Escolha entre as 5 identidades visuais oficiais do Zenith.
          </p>
        </DialogHeader>
        ...
        <div className="space-y-5 pt-2">
          ...
          {/* Seletor dos 5 Temas Oficiais */}
          <div className="space-y-2.5">
            <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
              Os 5 Temas Oficiais do Zenith
            </span>
              ...
              {PALETTES.map((p) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-3.5 rounded-2xl border p-3 text-left transition-all ${
                      isSelected
                        ? "border-primary bg-primary/10 shadow-sm ring-1 ring-primary"
                        : "border-border/50 bg-background/40 hover:bg-secondary/60"
                    }`}
                  >
                    {/* Logotipo em Miniatura Oficial do Tema */}
                    <div className="relative shrink-0 flex items-center justify-center">
                      <ZenithLogo size={44} palette={p.id} variant="badge" />
                    </div>
